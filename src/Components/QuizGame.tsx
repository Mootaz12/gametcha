...
const QuizGame: React.FC = () => {
  ...
  const handleNextQuestion = () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      // Automatically trigger start new game after quiz completion
      setTimeout(() => {
        setStartNewGame(true); // Start a new game after quiz completion
      }, 2000); // Wait for 2 seconds before starting the new game
    }
  };
    ...
    <div className="flex flex-col items-center space-y-6 p-6 border rounded-md shadow-md bg-white w-96">
      ...
      {currentQuestionIndex < questions.length ? (
        ...
        <>
          ...
          {/* The new game is triggered automatically here */}
          <div className="text-center mt-4">Starting a new game...</div>
        </>
      )}
    </div>
  );
